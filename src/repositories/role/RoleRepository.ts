...
export class RoleRepository extends BaseRepository<IRole> {
  ...
  async findByRole(role: string): Promise<IRole | null> {
    ...
    if (error) {
      throw new ApiError('Failed to find role', StatusCodes.INTERNAL_SERVER_ERROR, 'RoleRepository');
    }
    if (!data) {
      throw new ApiError('Role not found', StatusCodes.NOT_FOUND, 'RoleRepository');
    }
  ...
  async findAllRoles(): Promise<IRole[]> {
    ...
    if (error) {
      throw new ApiError('Failed to find roles', StatusCodes.INTERNAL_SERVER_ERROR, 'RoleRepository');
    }
