...
export class UserRepository extends BaseRepository<IUser> {
  ...
  private async createIndexes(): Promise<void> {
    await this.collection.createIndex({ lastActiveAt: 1 }, { expireAfterSeconds: 60 * 60 * 24 * 30, background: true });
  }
  ...
  async incrementFailedAttempts(userId: string): Promise<void> {
    return this.runWithErrorHandling(async () => {
      await this.collection.updateOne({ _id: new ObjectId(userId) } as Condition<IUser>, {
  ...
  async updateLastLogin(userId: string, deviceInfo?: Request['deviceInfo']): Promise<void> {
    return this.runWithErrorHandling(async () => {
      ...
      const result = await this.collection.updateOne({ _id: new ObjectId(userId) } as Condition<IUser>, {
        $set: updateData
      });
  ...
  async updateActivity(userId: string, deviceInfo?: Request['deviceInfo']): Promise<void> {
    return this.runWithErrorHandling(async () => {
      ...
      const result = await this.collection.updateOne({ _id: new ObjectId(userId) } as Condition<IUser>, { $set: updateData });
      if (result.modifiedCount === 0) {
        throw new ApiError('Failed to update user activity', StatusCodes.INTERNAL_SERVER_ERROR, 'UserRepository');
      }
    ...
    return this.runWithErrorHandling(async () => {
      const pipeline = [
        { $match: { _id: new ObjectId(userId) } },
        {
          $project: {
            inactiveMinutes: {
              $divide: [{ $subtract: [new Date(), '$lastActiveAt'] }, 1000 * 60]
            }
          }
        }
      ];
