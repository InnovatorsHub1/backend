...
export class AuthService {
  ...
  private validateEmail(email: string): void {
    if (!StandardValidators.email(email)) {
      throw new ApiError('Invalid email format', StatusCodes.BAD_REQUEST, 'AuthService');
    }
  }
  ...
  async login(email: string, password: string, deviceInfo: Request['deviceInfo']): Promise<LoginResponse> {
    ...
    const user = await this.runWithErrorHandling(() => this.userRepository.findByEmail(email.toLowerCase()), 'Failed to fetch user');
    ...
    if (!isPasswordValid) {
      await this.runWithErrorHandling(() => this.handleFailedLogin(user._id!.toString()), 'Failed to update login attempts');
      throw new ApiError('Invalid credentials', StatusCodes.UNAUTHORIZED, 'AuthService');
    }
