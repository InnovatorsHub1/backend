...
class TestHealthRoutes:
    ...
    def test_get_detailed_health_success(self, health_app, client, headers):
        ...
        assert all(key in data["system_metrics"] for key in ["cpu", "memory", "disk"])
        assert all(key in data["application_metrics"] for key in [
            "uptime_seconds", "user_count", "deleted_user_count", "python_process"
        ])
